...
======================
Trains a CatBoostRegressor to predict Station-Level Hourly Demand (total_energy_wh).

Pipeline:
  1. Load pre-split features
  2. Train CatBoost Regressor
  3. Evaluate: RMSE, MAE, R2, MAPE
  4. Visualise: Feature importance, Actual vs Predicted, Residuals, SHAP
  5. Save model → models/catboost_demand_model.cbm
"""
...
MODEL_FILE   = os.path.join(MODEL_DIR, "catboost_demand_model.cbm")
RANDOM_SEED  = 42
...
def load_data(filepath: str):
    print(f"\n{'='*60}")
    print("  CatBoost — Station-Level Hourly Demand Prediction")
    print(f"{'='*60}\n")
    if not os.path.exists(filepath):
        raise FileNotFoundError(f"[ERROR] '{filepath}' not found. Run prepare_demand_features.py first.")
...
def evaluate_model(model, X_test, y_test):
    ...
    metrics_path = os.path.join(OUTPUT_DIR, "catboost_demand_metrics.txt")
    with open(metrics_path, "w") as f:
        f.write("CatBoost — Station-Level Hourly Demand Prediction\n")
        f.write("=" * 40 + "\n")
...
def plot_feature_importance(model, feature_cols):
    ...
    plt.title("CatBoost (Demand) — Feature Importances", fontsize=13)
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, "catboost_demand_feature_importance.png"), dpi=150)
    plt.close()

def plot_actual_vs_predicted(y_test, y_pred):
    plt.figure(figsize=(8, 7))
    plt.scatter(y_test, y_pred, alpha=0.3, s=15, color="darkorange")
    lims = [0, max(y_test.max(), y_pred.max())]
    plt.plot(lims, lims, "r--", linewidth=1.5)
    plt.xlabel("Actual Total Energy (Wh)")
    plt.ylabel("Predicted Total Energy (Wh)")
    plt.title("CatBoost (Demand) — Actual vs Predicted", fontsize=13)
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, "catboost_demand_actual_vs_predicted.png"), dpi=150)
    plt.close()

def plot_residuals(y_test, y_pred):
    residuals = y_test - y_pred
    plt.figure(figsize=(9, 5))
    sns.histplot(residuals, bins=80, kde=True, color="darkorange")
    plt.axvline(0, color="black", linestyle="--")
    plt.xlabel("Residual (Actual - Predicted) Wh")
    plt.title("CatBoost (Demand) — Residual Distribution", fontsize=13)
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, "catboost_demand_residuals.png"), dpi=150)
    plt.close()
...
def plot_shap(model, test_pool, feature_cols):
    ...
    plt.title("CatBoost (Demand) — SHAP Feature Contributions", fontsize=12)
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, "catboost_demand_shap_summary.png"), dpi=150, bbox_inches="tight")
    plt.close()
